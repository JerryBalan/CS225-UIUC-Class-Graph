...
fdgOutput::fdgOutput(int version, Graph graph, unsigned iterations) {
    if(version == 0)
        defineLocationsSerial(graph, 5, iterations, graph.getVertices().size());
    else
        defineLocationsParallel(graph, 5, iterations, graph.getVertices().size());
}

fdgOutput::fdgOutput(int version, Graph graph, int scale, unsigned iterations, int classAmnt) {
    if(version == 0)
        defineLocationsSerial(graph, scale, iterations, classAmnt);
    else
